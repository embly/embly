...
func testUserToManyFunctions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, functionDBTypes, false, functionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, functionDBTypes, false, functionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Functions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFunctions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Functions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Functions = nil
	if err = a.L.LoadFunctions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Functions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpFunctions(t *testing.T) {
	var err error
	...
	foreigners := []*Function{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, functionDBTypes, false, strmangle.SetComplement(functionPrimaryKeyColumns, functionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Function{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddFunctions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Functions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Functions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Functions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Username`: `character varying`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
