...
// Functions retrieves all the function's Functions with an executor.
func (o *User) Functions(mods ...qm.QueryMod) functionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"functions\".\"user_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"functions\".*"})
	}

	return query
}

// LoadFunctions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFunctions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`functions`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load functions")
	}

	var resultSlice []*Function
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice functions")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on functions")
	}
	...
	if singular {
		object.R.Functions = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Functions = append(local.R.Functions, foreign)
				if foreign.R == nil {
					foreign.R = &functionR{}
				}
...
// AddFunctions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Functions.
// Sets related.R.User appropriately.
func (o *User) AddFunctions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Function) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Functions = append(o.R.Functions, related...)
	}
